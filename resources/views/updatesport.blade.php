...
<body>
    ...
    @foreach($data as $d)
        <form action="{{URL::to('/usport/'.$d->id)}}" method="post">
            @csrf
     
@if(session()->has('successs'))
  <h3 class="su">{{session('successs')}}</h3>
@endif
  <span id="ei">@error('en'){{$message}}@enderror</span>
            <input type="text" placeholder="{{__('welcome.heading36')}}" name="en" value="{{$d->sportname}}"
          ...
          <span id="ei">@error('semname'){{$message}}@enderror</span>
            <input type="text" name="semname" id="" value="{{$d->semname}}"
            class="{{$errors->first('semname')?'ie':''}}">
           ...
           <span id="ei">@error('tc'){{$message}}@enderror</span>
            <input type="text" placeholder="{{__('welcome.heading37')}}" name="tc" value="{{$d->complaintdetails}}"
